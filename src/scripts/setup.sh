...
ALEXASRC_DIRECTORY_CORRECT="/opt/space/src"
CONFIG_SYSTEM_DIRECTORY="/etc/opt/space"
...
if [ "$ALEXASRC_DIRECTORY" != "$ALEXASRC_DIRECTORY_CORRECT" ]; then
    ...
    echo "You haven't downloaded AlexaPi into /opt. As a result of that, you won't be able to run spacePi on boot."
    echo "If you wish to be able to run spacePi on boot, please interrupt this script and download into /opt."
    echo ""
...
if [ "$SPACESRC_DIRECTORY" == "$ALEXASRC_DIRECTORY_CORRECT" ]; then
...
echo "If you use a desktop OS, you HAVE TO set up your system audio so services like spacePi can use it too."
echo "See https://github.com/Space-Monkey-KD/space/wiki/Audio-setup-&-debugging#pulseaudio"
echo "######################################################################################################"
echo ""
